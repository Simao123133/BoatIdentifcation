%Generate montargil data
clear
load('Montargil16-46-59.mat');
%% Organize variables
state(1) = arrange(VESC1_RPM_MC_50_V1_RPM_02);
state(1).val = state(1).val;
state(2) = arrange(Foils_US_ang_38_right_angle_03);
state(3).val = 1.91*ones(length(state(1).time),1);
state(3).time = state(1).time;
state(4) = arrange(XCDI_Velocit_60_velX_00);
state(5) = arrange(XCDI_Velocit_60_velZ_02);
state(6) = arrange(XCDI_RateOfT_57_gyrY_01);
state(6).val = state(6).val*180/pi;
state(7) = arrange(XCDI_EulerAn_54_pitch_00);
state(8) = arrange(Foils_US_ang_38_dist_01);
state(9) = arrange(XCDI_Velocit_60_velY_01);
state(10) = arrange(XCDI_RateOfT_57_gyrX_00);
state(10).val = state(10).val*180/pi;
state(11) = arrange(XCDI_RateOfT_57_gyrZ_02);
state(11).val = state(11).val*180/pi;
state(12) = arrange(XCDI_EulerAn_54_roll_01);
state(13) = arrange(XCDI_EulerAn_54_yaw_02);
state(14) = arrange(Foils_US_ang_38_left_angle_02);

late_i = state(4).time(800000);
...
gyrX = s.gyrX;                 
gyrZ = s.gyrZ;  
save('raw_data_montargil.mat','Motor','Right_foil','Rear_foil','U','W','gyrY','Pitch','Height','V','gyrX','gyrZ','Roll','Yaw','Left_foil','Rudder');
